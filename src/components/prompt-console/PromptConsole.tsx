...
export function PromptConsole({
  ...
  return (
    <div className="flex flex-col h-full bg-[#121214] p-4 overflow-y-auto space-y-4">
      {/* Header */}
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          <Wand2 className="w-4 h-4 text-zinc-300" />
          <h2 className="text-sm font-semibold text-[#ededed] tracking-wide">
        ...
        <button
          ...
          className="text-[11px] text-zinc-400 hover:text-white flex items-center gap-1 px-2.5 py-1 rounded bg-[#18181b] border border-[#27272a] hover:border-zinc-500 transition cursor-pointer"
            ...
            <button
              ...
              className={`p-2.5 rounded-xl border text-left transition cursor-pointer flex flex-col justify-between group ${
                prompt === preset.prompt
                  ...
                  : 'border-[#27272a] bg-[#18181b] hover:bg-[#27272a]/50 text-zinc-300'
              }`}
            >
      ...
      <div className="space-y-1.5 flex-1 flex flex-col">
        ...
        <div className="relative flex-1 min-h-[140px]">
          <textarea
            ...
            className="w-full h-full min-h-[140px] bg-[#18181b] border border-[#27272a] rounded-xl p-3.5 text-xs text-zinc-100 placeholder:text-zinc-600 focus:outline-none focus:border-zinc-500 resize-y leading-relaxed font-sans"
          />
        </div>
          ...
          <div className="flex items-center gap-1.5 flex-wrap pt-1">
            ...
            {assets.map((a) => (
              <button
                ...
                className="px-1.5 py-0.5 rounded text-[10px] font-mono font-medium bg-[#18181b] hover:bg-[#27272a] text-zinc-300 border border-[#27272a] hover:border-zinc-500 transition cursor-pointer"
              >
                +{a.id}
              </button>
            ))}
      ...
      <div className="p-3 rounded-xl bg-[#18181b] border border-[#27272a] flex flex-col sm:flex-row items-start sm:items-center justify-between gap-3">
        <div className="flex items-center gap-2">
          <Sliders className="w-4 h-4 text-zinc-400" />
          <div>
            <div className="text-xs font-semibold text-zinc-200">Editing Pacing Style</div>
          ...
          {[
            ...
            <button
              ...
              className={`px-2.5 py-1 rounded text-[11px] font-medium transition cursor-pointer ${
                pacing === p.id
                  ? 'bg-[#27272a] border border-zinc-500 text-[#ededed]'
      ...
      {/* Error Message */}
      {errorMsg && (
        <div className="p-3 rounded-xl bg-zinc-900 border border-zinc-700 text-zinc-300 text-xs flex items-center gap-2">
      ...
      <button
        ...
        className={`w-full py-3 px-4 rounded-xl text-xs font-semibold flex items-center justify-center gap-2 transition cursor-pointer ${
          isLoading
            ? 'bg-[#18181b] text-zinc-500 border border-[#27272a] cursor-wait'
            : 'bg-[#27272a] hover:bg-[#3f3f46] text-[#ededed] border border-[#3f3f46] active:scale-[0.99]'
        }`}
      >
        {isLoading ? (
          <>
      ...
      {currentTimeline && (
        <div className="p-3 rounded-xl bg-[#18181b] border border-[#27272a] text-xs text-zinc-400 space-y-1">
